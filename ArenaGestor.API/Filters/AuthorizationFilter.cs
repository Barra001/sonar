...
namespace ArenaGestor.API.Filters
{
    public class AuthorizationFilter : Attribute, IAuthorizationFilter
    {
        ...
        {
            securityService = context.HttpContext.RequestServices.GetService<ISecurityService>();
            ...
            try
            {
                if (securityService.GetUserOfToken(token) == null)
                {
                    context.Result = new ContentResult()
                    {
            ...
            {
                if (securityService.UserHaveRole(item, token))
                {
                    isAuth = true;
                }
            }
